		...
		<table class="table table-bordered table-hover">
			...
			<c:forEach items="${list}" var="freeBoard">
				<tr>
					...
					<c:choose>
					    <c:when test="${freeBoard.field eq '0'}"><td>반려동물 자랑</td></c:when>
					    <c:when test="${freeBoard.field eq '1'}"><td>사료/간식 추천</td></c:when>
					    <c:when test="${freeBoard.field eq '2'}"><td>병원후기</td></c:when>
					    <c:when test="${freeBoard.field eq '3'}"><td>투병일기</td></c:when>
					    <c:when test="${freeBoard.field eq '4'}"><td>산책회</td></c:when>
					</c:choose>
