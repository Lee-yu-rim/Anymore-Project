...
public class ReplyController {
	...
	public ResponseEntity<List<ReplyVO>> getList(@PathVariable("page") int page, @PathVariable("bno") Long bno) {
		...
		log.info(cri);
		return new ResponseEntity<>(service.pagingReply(cri, bno), HttpStatus.OK);
	}
	...
	public ResponseEntity<ReplyVO> get(@PathVariable("rno") Long rno) {
		log.info("get : " + rno);
		return new ResponseEntity<ReplyVO>(service.listReply(rno), HttpStatus.OK);
	}
	...
	public ResponseEntity<String> modify(@RequestBody ReplyVO reply, @PathVariable("rno") Long rno){
		...
		return service.updateReply(reply) == 1 ? new ResponseEntity<String>("success", HttpStatus.OK) :
			  							 new ResponseEntity<String>(HttpStatus.INTERNAL_SERVER_ERROR);
	}
