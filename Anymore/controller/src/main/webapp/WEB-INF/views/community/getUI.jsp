   ...
   <div class="container">
      ...
      <div class="row">
      	 <div class="col-md-3">
	      	 <b>카테고리</b>&nbsp; 
	      	 <c:choose>
			    <c:when test="${usedItems.field eq '팝니다'}">팝니다</c:when>
			    <c:when test="${usedItems.field eq '삽니다'}">삽니다</c:when>
			    <c:when test="${usedItems.field eq '후기'}">후기</c:when>
			</c:choose>
      	 </div>
...
<!-- 댓글처리 -->
<script type="text/javascript" src="../js/replyUI.js"></script>

<!-- 댓글 전체 목록 보여주기 -->
<script>
   $(function(){   
      var bnoValue = '<c:out value="${usedItems.bno}"/>';
        var replyUL = $(".chat");
